...
def process_annotation():
    try:
        ...
        if source == "pc":
            ...
            label = get_label(error)
            confidence = 100 - (100 * float(error))#rubric function
            diagnosis = build_diagnosis(error, confidence)
...
def build_diagnosis(error, confidence):
    if confidence < 100 and confidence > 98 and error > threshold:
        ...
        diagnosis = 'My analysis concludes there could be an anomaly in the structure, but either the structure is not well annotated or the image could be distorted, small, unclear or bad hence [...]'
    elif confidence < 96 and confidence > 92 and error > threshold:
        diagnosis = 'High possibility that anomaly is false psoitive and image is greatly distorted or irrelevant. Please check the image or the annotation and try again. If results are same, con[...]'
